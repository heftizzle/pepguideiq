...
- **Affiliate codes (14 whitelisted).** `?ref=` values are normalized case-insensitively by `normalizeAffiliateRef()` in `src/lib/affiliateRef.js`; unknown Rewardful codes are dropped. One affiliate may have multiple active codes (Rewardful tracks per code). Canonical codes: Primo15, Pete15, Tsource15, EDON15, HEAVYDUTY15, ironresolve15, Palmer15, Ryba15, Fire15, Lake15, OTMen15, Elite15, Vitality15 (Dr. Tracy / Live In Vitality, Riverview, FL), Chad15.
...
- Do not invent Worker routes. There are exactly 34 — see `workers/CLAUDE.md`.
