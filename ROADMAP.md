...
> Built in ~37 days. 264-compound catalog across 36 batches. 89 Supabase migrations.
> Live at [pepguideiq.com](https://pepguideiq.com)
...
- [x] Rewardful affiliate tracking — 14+ codes whitelisted, case-insensitive; referral ID capture + webhook UUID resolution
...
- [x] Cancel / reactivate subscription flow — Worker + modal copy ("Awww... but we like money. 🥺")
...
- [ ] Likes + comments + notifications on feed posts ✅ *(shipped pre-launch)*
...
- [ ] BATCH37+ compound additions (queued, catalog frozen at 264 pre-launch)

---

## 🧠 V2 — AI Atlas Memory Layer (RAG + Agent)
> The moat. Johnny Hughes is the architect on this.

This is the vision that makes pepguideIQ unreplicable at $9/mo consumer pricing.
...
1. **pgvector in Supabase** — enable extension, design embeddings schema around
   user protocol data (dose logs, InBody entries, stack events, compound interactions)
2. **Embedding pipeline** — every log entry becomes a vector; Johnny has prior experience
   with this pipeline pattern
3. **RAG retrieval** — Atlas pulls user's own data as context before every response;
   compound catalog also embedded for reference
4. **Memory synthesis** — Atlas makes personalized calls:
...
5. **Custom AI Agent** — full personalized profile management, not just Q&A;
   proactive protocol suggestions based on logged trends

**Why this is the moat:**
- The user's data is the context. Nobody else has it.
- Every log entry makes Atlas smarter for that user specifically.
- At the Coach Tier ($50/mo) this becomes a clinical-grade tool for TRT clinics
  and functional medicine practices — a legitimate B2B SaaS wedge.
- The social feed flywheel reinforces it: log to share → log to track → log to teach Atlas.
  Three reasons to log. One action.

**Domain:** pepguideiq.ai (secured) becomes the primary URL when this ships.
That's not just branding — it's accurate. It describes the architecture.
...
- [ ] **Coach Tier — $50/mo** — handle-based client roster, permission-based stack visibility,
  assign protocols to clients, RAG-powered coaching context
...
- [ ] REEL DOSES — receipted photo/video tied to dose log entries; injection content
  Instagram cannot host. This is the V2 kill shot for influencer growth.
- [ ] PWA offline — catalog, vials, calculator offline; AI Atlas requires connectivity
- [ ] Compound PDF library — 2-3 page PDFs per compound (consistent schema),
  feeds the RAG pipeline; v3 = personalized stack report PDF for GOAT tier
- [ ] Daily movie quote — in-app dose-logging reward
- [ ] Oasis purity data integration
- [ ] Finnrick deep integration / data partnership (outreach queued for Week 1 post-launch)
...
- [ ] pepguideiq.ai DNS flip (domain secured, timing: when RAG ships)
...
## 🌐 Domain Portfolio
> Full brand moat secured

| Domain | Purpose |
|---|---|
| pepguideiq.com | Primary — live now |
| pepguideiq.ai | Future primary — flip when RAG/Atlas ships |
...
**Partnership tier (not affiliate):** Jacob Nachinson (Overtime, 157K) + partner (293K)
**Prospect:** Josh Holyfield (93K), Jay Cutler, Dee (@deemariedi, 393K)
**Queued Week 1:** Finnrick outreach
...
- [x] 264-compound catalog across 36 batches (BATCH1-36); BATCH37 queued post-launch
...
- [x] Cancel / reactivate subscription (Worker + "Awww... but we like money. 🥺" modal)
...
- [x] Rewardful affiliate integration — 14 codes, whitelist mode
- [x] DMARC + SPF + DKIM email auth stack
- [x] Liability insurance — E&O + GL + Cyber (Next Insurance)