...
var VALID_PLAN_TIERS = /* @__PURE__ */ new Set(["entry", "pro", "elite", "goat"]);
var TIER_RANK = { entry: 0, pro: 1, elite: 2, goat: 3 };
...
__name(memberProfileSlotLimit, "memberProfileSlotLimit");
var DAILY_QUERY_LIMIT = {
  entry: 2,
  pro: 4,
  elite: 8,
  goat: 16
};
var DAILY_ADVISOR_LIMIT = {
  entry: 5,
  pro: 20,
  elite: 30,
  goat: 40
};
var MODEL_ENTRY_PRO = "claude-haiku-4-5-20251001";
var MODEL_ELITE_GOAT = "claude-sonnet-4-6";
var STACK_PHOTO_MAX_BYTES = 10 * 1024 * 1024;
var STACK_PHOTO_TYPES = /* @__PURE__ */ new Map([
  ["image/jpeg", "jpg"],
  ["image/png", "png"],
  ["image/webp", "webp"],
  ["image/gif", "gif"]
...
__name(contentTypeFromKey, "contentTypeFromKey");
...
var SECURITY_HEADERS = {
  ...
  "Content-Security-Policy": "default-src 'self'; script-src 'self' 'unsafe-inline' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://*.r2.cloudflarestorage.com https://*.workers.dev; connect-src 'self' https://api.anthropic.com https://*.supabase.co https://*.workers.dev; frame-src https://challenges.cloudflare.com; object-src 'none'; base-uri 'self'; form-action 'self'"
};
var RATE_LIMIT_STORE = /* @__PURE__ */ new Map();
var RATE_LIMITS = {
  auth: { windowMs: 6e4, max: 10 },
  api: { windowMs: 6e4, max: 60 },
  /** Authed image uploads (POST/PUT): 20/min per IP. */
  r2_write: { windowMs: 6e4, max: 20 },
  /** Authenticated private image reads (GET /stack-photo): 500/min per IP. Public /avatars/ is unrate-limited. */
  r2_read: { windowMs: 6e4, max: 500 }
};
...
function checkRateLimit(ip, type) {
  ...
  if (entry.count > cfg.max) {
    const retryAfter = Math.max(1, Math.ceil((entry.windowStart + cfg.windowMs - now) / 1e3));
...
__name(publicAvatarCorsHeaders, "publicAvatarCorsHeaders");
...
async function verifyStripeWebhookSignature(rawBody, sigHeader, secret) {
  ...
  const tsMs = Number(t) * 1e3;
  if (Number.isFinite(tsMs)) {
    const maxSkew = 5 * 60 * 1e3;
...
__name(fetchStripeSubscriptionExpanded, "fetchStripeSubscriptionExpanded");
...
async function handleStackAdvisor(request, env) {
  ...
  try {
    ...
    const compactCatalog = catalog.filter((c) => c && typeof c.id === "string" && !stackIds.has(c.id)).map((c) => ({
      id: c.id,
...
Assess the stack and return exactly 4 recommendations in the required JSON shape (each with peptideId, name, reason, and tier).`
    ...
    const ADVISOR_TIERS = /* @__PURE__ */ new Set(["must_have", "nice_to_have", "not_necessary", "redundant"]);
    const rawRecs = Array.isArray(parsed.recommendations) ? parsed.recommendations : [];
    const safeRecs = rawRecs.map((r) => {
      if (!r || typeof r !== "object") return null;
      const pid = typeof r.peptideId === "string" ? r.peptideId.trim() : typeof r.catalogId === "string" ? r.catalogId.trim() : "";
      if (!pid || !catalogIdSet.has(pid)) return null;
      const tierRaw = typeof r.tier === "string" ? r.tier.trim().toLowerCase().replace(/-/g, "_") : "";
      const tier = ADVISOR_TIERS.has(tierRaw) ? tierRaw : void 0;
      const name = typeof r.name === "string" && r.name.trim() ? r.name.trim() : pid;
      const reason = typeof r.reason === "string" ? r.reason.trim() : typeof r.rationale === "string" ? r.rationale.trim() : "";
...
__name(handleStackAdvisor, "handleStackAdvisor");
...
__name(modelForPlan, "modelForPlan");
async function checkDailyQueryRateLimit(env, kv, userId, plan) {
  ...
  await kv.put(key, JSON.stringify(next), { expirationTtl: 9e4 });
  return { allowed: true, count: next.count, limit };
}
__name(checkDailyQueryRateLimit, "checkDailyQueryRateLimit");
...
async function getSessionUser(env, authorization) {
  ...
  const user = body && typeof body === "object" && typeof body.id === "string" ? body : body && typeof body === "object" && body.user && typeof body.user.id === "string" ? body.user : null;
  if (!user) {
    return { data: null, error: new Error("Unauthorized") };
  }
...
__name(supabaseAuthReady, "supabaseAuthReady");
...
__name(fetchProfilePlan, "fetchProfilePlan");
var AI_GUIDE_SYSTEM_BASE = "You are an expert peptide research advisor with deep knowledge of peptide pharmacology, biohacking protocols, dosing strategies, and interactions. Be direct, technical, and practical. Always include safety notes \u2014 these are research chemicals requiring physician oversight.";
...
__name(buildChatSystemBlocks, "buildChatSystemBlocks");
...
__name(injectCatalogCacheIntoMessages, "injectCatalogCacheIntoMessages");
function tierPlanFromStripeSubscription(subscr) {
  ...
  const m = typeof subscr.metadata?.plan === "string" && subscr.metadata.plan.trim().toLowerCase() || typeof subscr.items?.data?.[0]?.price?.metadata?.plan === "string" && subscr.items.data[0].price.metadata.plan.trim().toLowerCase() || "";
  return VALID_PLAN_TIERS.has(m) ? m : "entry";
}
__name(tierPlanFromStripeSubscription, "tierPlanFromStripeSubscription");
...
__name(supabasePatchProfile, "supabasePatchProfile");
...
__name(supabasePatchUserVial, "supabasePatchUserVial");
...
__name(supabasePatchMemberProfile, "supabasePatchMemberProfile");
function memberProfilePatchFailureMessage(result, patch) {
  ...
  if (p && typeof p === "object" && !Array.isArray(p)) {
    ...
    if (code === "23505" || /duplicate key|unique constraint/i.test(message) || /already exists/i.test(details)) {
...
__name(memberProfilePatchFailureMessage, "memberProfilePatchFailureMessage");
  ...
  if (!insStack.ok) {
    ...
    if (insStack.status === 409 || /unique|duplicate/i.test(errDetail)) {
      if (/user_stacks_pkey/i.test(errDetail) && /\(user_id\)/i.test(errDetail)) {
        hint = " Run supabase/migrations/015_user_stacks_pkey_on_id.sql \u2014 PRIMARY KEY is on user_id; it must be on id, with UNIQUE (user_id, profile_id).";
      } else {
        hint = " Ensure 014_user_stacks_profile_unique_fix.sql ran (drop legacy UNIQUE user_id only). If needed, also run 015_user_stacks_pkey_on_id.sql.";
      }
    }
    return jsonResponse(
      {
...
__name(handleCreateMemberProfile, "handleCreateMemberProfile");
...
__name(supabaseSyncUserPlanAndCustomer, "supabaseSyncUserPlanAndCustomer");
async function handleStripeSubscription(request, env, cors) {
  ...
  const customerId = profile && typeof profile.stripe_customer_id === "string" ? profile.stripe_customer_id.trim() : "";
  const emptyResponse = /* @__PURE__ */ __name(() => jsonResponse({
    current_period_end: 0,
    cancel_at_period_end: false,
    status: "none",
    plan: "entry",
    pending_plan: profile?.pending_plan != null ? String(profile.pending_plan) : null
  }, 200, cors), "emptyResponse");
  if (!customerId) {
...
__name(handleStripeSubscription, "handleStripeSubscription");
async function handleScheduleDowngrade(request, env, cors) {
  ...
  const ok = await supabasePatchProfile(supabaseUrl, serviceKey, userId, {
    pending_plan: target,
    pending_plan_date: new Date(subscr.current_period_end * 1e3).toISOString()
  });
  if (!ok) {
...
__name(handleScheduleDowngrade, "handleScheduleDowngrade");
...
__name(handleUploadStackPhoto, "handleUploadStackPhoto");
async function handleGetStackPhoto(request, env, cors) {
  ...
  try {
    ...
    const ct = obj.httpMetadata && typeof obj.httpMetadata.contentType === "string" && obj.httpMetadata.contentType || contentTypeFromKey(key);
...
__name(handleGetStackPhoto, "handleGetStackPhoto");
var UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
var AVATAR_R2_KEY_RE = new RegExp(
...
__name(handleGetAvatar, "handleGetAvatar");
var MEMBER_PROFILE_ALLOWED_LANGUAGES = /* @__PURE__ */ new Set(["en", "es", "pt-BR", "fr", "de", "ja", "zh-Hans"]);
var MEMBER_PROFILE_SHIFT_SCHEDULES = /* @__PURE__ */ new Set(["days", "swings", "mids", "nights", "rotating"]);
var MEMBER_HANDLE_RE = /^[a-z0-9_]{3,20}$/;
var MEMBER_EXPERIENCE_LEVELS = /* @__PURE__ */ new Set(["beginner", "intermediate", "advanced", "elite"]);
function normalizeHandleForPatch(v) {
  let t = String(v).trim().toLowerCase();
  if (t.startsWith("@")) t = t.slice(1);
  return t;
}
__name(normalizeHandleForPatch, "normalizeHandleForPatch");
...
__name(isHandleTakenElsewhere, "isHandleTakenElsewhere");
async function handleCheckHandleAvailability(request, env, cors) {
  ...
  const excludeProfileId = UUID_RE.test(excludeRaw) ? excludeRaw : void 0;
  if (!h) {
...
__name(handleCheckHandleAvailability, "handleCheckHandleAvailability");
...
__name(normalizeWakeTimeForPatch, "normalizeWakeTimeForPatch");
...
__name(handleGetMemberProfiles, "handleGetMemberProfiles");
function parseMemberProfilePatchBody(body) {
  ...
  const setNullableText = /* @__PURE__ */ __name((key, maxLen) => {
...
__name(parseMemberProfilePatchBody, "parseMemberProfilePatchBody");
...
__name(handlePatchMemberProfile, "handlePatchMemberProfile");
...
__name(handleDeleteMemberProfile, "handleDeleteMemberProfile");
async function handlePostStackPhoto(request, env, cors) {
  ...
  const memberProfileId = typeof memberProfileIdRaw === "string" ? memberProfileIdRaw.trim() : "";
  if (kind === "body_scan") {
    const profilePlan = await fetchProfilePlan(supabaseUrl, serviceKey, userId);
    if (TIER_RANK[profilePlan] < TIER_RANK.pro) {
  ...
  const memberScopedKinds = /* @__PURE__ */ new Set(["body_scan", "progress_front", "progress_side", "progress_back"]);
  if (!["stack_shot_1", "stack_shot_2", "vial", "avatar", ...memberScopedKinds].includes(kind)) {
    return jsonResponse(
      {
        error: "kind must be stack_shot_1, stack_shot_2, vial, avatar, body_scan, progress_front, progress_side, or progress_back"
      },
      400,
      cors
    );
...
__name(handlePostStackPhoto, "handlePostStackPhoto");
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const userId = typeof session.client_reference_id === "string" && session.client_reference_id || typeof session.metadata?.supabase_user_id === "string" && session.metadata.supabase_user_id || "";
        const customerId = typeof session.customer === "string" ? session.customer : session.customer && typeof session.customer === "object" ? session.customer.id : "";
        let plan = typeof session.metadata?.plan === "string" && normalizePlanTier(session.metadata.plan) || "entry";
        if (userId && customerId) {
          const subId = typeof session.subscription === "string" ? session.subscription : session.subscription && typeof session.subscription === "object" ? session.subscription.id : "";
      ...
      case "customer.subscription.deleted": {
        ...
        const plan = event.type === "customer.subscription.deleted" ? "entry" : tierPlanFromStripeSubscription(sub);
        const ok = await supabaseSyncUserPlanAndCustomer(env, supabaseUrl, serviceKey, userId, plan, customerId);
        if (!ok) {
...
async function handleRequest(request, env) {
  ...
  const skipRateLimitPublicAvatar = pathname.startsWith("/avatars/") && (method === "GET" || method === "HEAD" || method === "OPTIONS");
  const isR2Write = pathname === "/stack-photo" && method === "POST" || pathname === "/upload-stack-photo" && method === "POST" || (pathname === "/avatars" || pathname === "/avatars/") && (method === "POST" || method === "PUT");
  ...
  if (!skipRateLimitPublicAvatar) {
    ...
    if (isR2Write) {
      ...
      rlType = "r2_read";
    } else if (pathname.startsWith("/auth") || pathname.startsWith("/login") || pathname.startsWith("/signup")) {
      rlType = "auth";
    } else {
      rlType = "api";
    }
  ...
  if ((stackAdvisorPath === "/ai-guide" || stackAdvisorPath === "/ai-stack-advisor" || stackAdvisorPath === "/stack-advisor") && request.method === "POST") {
