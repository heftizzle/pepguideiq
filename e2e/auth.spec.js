...
import { getEmailInput, getPasswordInput, loginUser, passAgeGateIfPresent } from "./helpers/auth.js";
...
test.describe("auth flows", () => {
  ...
  test("bad password shows an error", async ({ page }) => {
    ...
    await getEmailInput(page).fill(process.env.E2E_TEST_EMAIL);
    await getPasswordInput(page).fill("wrong-password-xyz");
