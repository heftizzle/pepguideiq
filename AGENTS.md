...
- **Affiliate codes (12 whitelisted).** Canonical codes (all 15% flat, case-insensitive via `normalizeAffiliateRef()` in `src/lib/affiliateRef.js`):
  ...
  All 12 codes are whitelisted in `normalizeAffiliateRef()`. Do not add codes not in this list. Old stale codes (KwElite15, OTMax15, Promo15) are retired.
...
- Do not invent Worker routes. There are exactly 25 — see `workers/AGENTS.md`.
